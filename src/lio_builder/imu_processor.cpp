...
namespace fastlio
{
    ...
    void IMUProcessor::init(const MeasureGroup &meas)
    {
        ...
        if (align_gravity_)
        {
            // 计算重力对齐旋转（IMU坐标系到世界坐标系）
            Eigen::Quaterniond gravity_align_rot = Eigen::Quaterniond::FromTwoVectors(
                (-mean_acc_).normalized(), 
                Eigen::Vector3d(0.0, 0.0, -1.0)
            );
            
            // Driver已经应用了15度的pitch旋转，需要补偿回来
            // 根据实际测试调整，改为15.0 - 0.81 = 14.19度
            // 通过绕Y轴旋转-14.19度来补偿
            const double driver_pitch_deg = 0.7115;  // 15.0 - 0.7115 = 14.2885度
            ...
            // 应用补偿：先重力对齐，再补偿driver的15度pitch
            state.rot = driver_comp_rot * gravity_align_rot;
